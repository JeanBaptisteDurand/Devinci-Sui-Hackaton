...
export function radialLayoutV2(
  graphData: GraphData,
  primaryPackageId?: string | null
): { nodes: Node[]; edges: Edge[] } {
  logger.debug('radialLayoutV2', 'Starting v2 layout calculation', {
  ...
  // Package layout: arrange in a circle with much more spacing
  const packageCount = packages.length;
  const packageRadius = Math.max(600, packageCount * 120); // Doubled spacing
  ...
  logger.info('radialLayoutV2', 'Layout complete', {
